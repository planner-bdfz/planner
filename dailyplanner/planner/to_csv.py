...
def change(cloud=0,path=os.path.dirname(__file__),seiue_id="",seiue_pswd=""):
    f=open(path+"/../user_dairy.csv","w",encoding="utf-8")
    wt=csv.writer(f);
    wt.writerow(["ID","标题","DDL","类型","重要标签值","重要基值","重要值","紧急参数","剩余时间","预计时间","紧急度","权重","起始时间","结束时间"])
    if(cloud):
        url="https://passport.seiue.com/login?school_id=3&type=account&force=1";
        s=requests.session();
        if(s.post(url,data={"email":seiue_id,"password":seiue_pswd,"school_id":3,"submit":"提交"}).status_code!=200):
            return 401;
        else:
